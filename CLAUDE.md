...
**AI Coding 面板**（2026-08-15 新增，决议见 `docs/situations/260815-ai-coding-panel.md`）：复刻自 nezha（**GPL-3.0**，自用无义务；若将来公开发布 ai-ssh，整仓需以 GPL-3.0 发布）的独立功能域——项目/任务管理 + Claude/Codex PTY 终端 + 文件浏览 + 本地 Shell + 看板。位于 client `src/features/aiCoding/`（前端）与 `src-tauri/src/coding/`（Rust），与 SSH 运维链路零共享：不经 ssh-server、不复用 SSH 信任链、Tauri 命令 `coding_` 前缀、事件 `coding:` 前缀、数据落 `~/.ai-ssh/coding/`、项目内目录 `.ai-coding/`。**独立于上述三条红线之外**：权限模式（ask/auto_edit/full_access）沿用 agent CLI 原生交互，由用户创建任务时显式选择。状态跟踪双路径 = hook 注入（改写用户 `~/.codex/config.toml`，Claude 走自有 settings 文件经 `--settings` 传入不改用户配置）+ agent session 文件轮询；Windows 全屏 TUI scrollback 依赖随包侧载的 ConPTY（`src-tauri/resources/conpty/`）。开发该功能需 `npm run tauri dev`（Rust command 参与编译）。

**已交付简史**：2026-08-25 · 手机伴侣恢复已结束任务（`POST /api/tasks/{id}/resume` 复刻 create job 模式直调 `coding_resume_task`，agent 会话续跑链路全复用；桌面联动三件套 task-status/web_created/task-resumed+跟随导航；横幅单击直达、无确认层，Esc 软打断兜底，detached 一期不开放；决议见 `docs/situations/260825-mobile-resume-task.md`）；2026-08-24 · 手机伴侣快照状态同步（建连秒开：服务端 vt100 无头仿真器惰性引导+常驻，快照从 256KB 尾窗全量重放改为最终屏幕状态序列 ~百字节，手机 ~10s→<1s；resize 纯跟随四点记账、硬失败回退原始尾窗、双端前端零 diff；决议见 `docs/situations/260824-mobile-snapshot-statesync.md` 与 done/260824 行动档；尾窗 tap 仍是回退与引导数据源）；2026-08-22 · AI Coding 手机伴侣一期（Tailscale tailnet + 进程内嵌 axum 18080 web 门面：token 鉴权 REST/WS、PTY 流尾窗回放+断线重连、手机 resize 接管/断开还原仲裁、新建任务+PC 视图跟随联动、keep-awake 防睡眠、自启开关、PWA-lite；桌面 emit×16 双写总线零回归；通道无关可换 frp；坑与决议见 `docs/situations/260821-mobile-companion.md` 与 done/260821 行动档）；2026-08-20 · AI Coding 资源评审修复 P1×4+P2×3+P3×3（PTY 毒字节防冻结、metrics 增量解析根治 O(N²)、watch 线程孤儿兜底、reset 清理对齐 finalize、删项目删目录、附件孤儿/log 启动清理、session 查看器 50MB 上限、shell 重开认代次、拖拽监听四重兜底；评审决议见 `docs/situations/260820-coding-resource-review.md`）；2026-08-17 · AI Coding 待确认桌面通知（input_required/awaiting_review 失焦时 Windows toast，点击进程内 Activated 回调跳转任务终端；identifier 切 `com.johnny.ai-ssh`；真机排障三坑见 done/260817 行动档，决议见 `docs/situations/260817-coding-desktop-notification.md`）；2026-08-15 · AI Coding 面板 v1（复刻 nezha：ActivityBar 入口 + 整窗接管 `centerView==="aiCoding"`，任务管理/PTY 双 agent/文件浏览/本地 Shell/看板，排除 git/timeline/skills/用量，CSS 隔离 + 电路霓虹色板映射，决议见 `docs/situations/260815-ai-coding-panel.md`）；更早：client UI 全量重设计「电路霓虹·能量流」（260727）、AI 会话生命周期整改（260723）、三开关体验优化+思考过程可视化（260723）、迭代 A/B/C1 安全地基与确认门（260721）、SFTP 双面板（ADR 0003）、Tool 调用闭环（ADR 0002）、single 形态落地（ADR 0001）、模型设置、UI 改版（Linear/Warp 风）。
