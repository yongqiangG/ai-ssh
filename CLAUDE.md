...
**已交付简史**：2026-08-22 · AI Coding 手机伴侣一期（Tailscale tailnet + 进程内嵌 axum 18080 web 门面：token 鉴权 REST/WS、PTY 流尾窗回放+断线重连、手机 resize 接管/断开还原仲裁、新建任务+PC 视图跟随联动、keep-awake 防睡眠、自启开关、PWA-lite；桌面 emit×16 双写总线零回归；通道无关可换 frp/自建 DERP；坑与决议见 `docs/situations/260821-mobile-companion.md` 与 done/260821 行动档）；2026-08-20 · AI Coding 资源评审修复 P1×4+P2×3+P3×3（PTY 毒字节防冻结、metrics 增量解析根治 O(N²)、watch 线程孤儿兜底、reset 清理对齐 finalize、删项目删目录、附件孤儿/log 启动清理、session 查看器 50MB 上限、shell 重开认代次、拖拽监听四重兜底；评审决议见 `docs/situations/260820-coding-resource-review.md`）；2026-08-17 · AI Coding 待确认桌面通知（input_required/awaiting_review 失焦时 Windows toast，点击进程内 Activated 回调跳转任务终端；identifier 切 `com.johnny.ai-ssh`；真机排障三坑见 done/260817 行动档，决议见 `docs/situations/260817-coding-desktop-notification.md`）；2026-08-15 · AI Coding 面板 v1（复刻 nezha：ActivityBar 入口 + 整窗接管 `centerView==="aiCoding"`，任务管理/PTY 双 agent/文件浏览/本地 Shell/看板，排除 git/timeline/skills/用量，CSS 隔离 + 电路霓虹色板映射，决议见 `docs/situations/260815-ai-coding-panel.md`）；2026-07-27 · client UI 全量重设计「电路霓虹·能量流」（仅深色 + motion 弹簧动效 + Chakra Petch/JetBrains Mono 本地打包；设计系统落 `ssh-client/DESIGN.md` + `.impeccable/design.json`，决议见 `docs/situations/260726-ui-neon-circuit-redesign.md`；注意 CSS Modules 内 keyframes 必须与消费方同模块）；更早：AI 会话生命周期整改（260723）、三开关体验优化+思考过程可视化（260723）、迭代 A/B/C1 安全地基与确认门（260721）、SFTP 双面板（ADR 0003）、Tool 调用闭环（ADR 0002）、single 形态落地（ADR 0001）、模型设置、UI 改版（Linear/Warp 风）。
